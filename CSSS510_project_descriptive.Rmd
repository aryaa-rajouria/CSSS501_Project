...
temp <- subset(df_all,
              !is.na(srh) & 
                !is.na(doc_status))
p1 <- ggplot(data = subset(df_all,!is.na(doc_status) & !is.na(srh)), aes(x=doc_status,
              fill=as.factor(srh))) +
...
p6 <- ggplot(data = subset(df_all,!is.na(gender) & !is.na(epd)),
             aes(x=gender,
              fill=as.factor(epd))) +
  geom_bar(position="fill", na.rm=T) +
  labs(y = "%",
...
desc_mh <- annotate_figure(mh_plot, 
                top = text_grob("Mental Health Outcomes",

                                                                          face = "bold", size = 14))

desc_srh <- annotate_figure(p1, 
                top = text_grob("Self-rated health",
                                          face = "bold", size = 14))
final_plot <- ggarrange(p1, desc_mh,
          ncol=1)
  
desc_mh
ggsave(filename="desc_mh", plot = desc_mh)
ggexport(desc_mh, filename = "desc_mh.png")
# annotate_figure(mh_plot, 
#                 top = text_grob("Mental health outcomes",
#                                           face = "bold", size = 14))

# save plot
png("desc_srh.png", width=450, height=180)
# 2. Create the plot
desc_srh
# 3. Close the file
dev.off()
